...
export default function EditEventCategoryPage() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const data = await getEventCategory(categoryId);
        setCategory(data);
      } catch (error) {
        console.error("Error fetching category:", error);
        toast({
          title: "Error",
          description: "Failed to fetch event category",
          variant: "destructive",
        });
  ...
  const handleSubmit = async (data: UpdateEventCategoryInput) => {
    setIsLoading(true);
    try {
      await updateEventCategory(categoryId, data);
      toast({
        title: "Success",
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8">
          <div className="text-center text-red-500">
            ...
            <p>
              The event category you&apos;re trying to edit doesn&apos;t exist.
            </p>
          </div>
        </div>
      </div>
