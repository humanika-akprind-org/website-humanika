...
export default function EventApprovalPage() {
  ...
  const [filters, setFilters] = useState({
    status: "",
    entityType: "EVENT", // Fixed to events only
  });
  ...
  const handleApprove = async (approvalId: string) => {
    try {
      ...
      if (response.error) {
        ...
        setSuccess("Event approval updated successfully");
        fetchApprovals();
  ...
  const clearFilters = () => {
    setFilters({ status: "", entityType: "EVENT" });
    setSearchTerm("");
    setCurrentPage(1);
  };
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-gray-600">Loading event approvals...</p>
          </div>
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-400 mt-1">
              {searchTerm || filters.status
                ...
                : "No event approval requests at the moment"}
            </p>
          </div>
        ) : (
          <>
