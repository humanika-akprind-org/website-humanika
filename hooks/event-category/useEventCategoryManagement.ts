...
import { deleteEventCategory } from "@/use-cases/api/event-category";
...
export function useEventCategoryManagement() {
  ...
  const { categories, isLoading, error, refetch } = useEventCategories();
  ...
  const [currentCategory, setCurrentCategory] = useState<EventCategory | null>(
    null
  );
  ...
  const handleAddCategory = () => {
    router.push("/admin/program/events/categories/add");
  };

  const handleEditCategory = (id: string) => {
    router.push(`/admin/program/events/categories/edit/${id}`);
  };
  ...
  const confirmDelete = async () => {
    try {
      if (currentCategory) {
        await deleteEventCategory(currentCategory.id);
