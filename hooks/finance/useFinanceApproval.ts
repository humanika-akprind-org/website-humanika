  ...
  const fetchApprovals = useCallback(async () => {
    try {
      ...
      if (response.error) {
        ...
        // Group approvals by finance ID and keep only the latest one per finance
        const uniqueApprovalsMap = response.data.approvals.reduce(
          (acc, approval) => {
            const financeId = approval.finance?.id || approval.entityId;
            if (
              !acc[financeId] ||
              new Date(approval.createdAt) > new Date(acc[financeId].createdAt)
            ) {
              acc[financeId] = approval;
            }
            return acc;
          },
  ...
  const handleReturn = async (approvalId: string) => {
    try {
      ...
      if (response.error) {
        ...
        setAlert({
          type: "success",
          message: "Finance approval returned to pending successfully",
        });
        fetchApprovals();
        setTimeout(() => setAlert(null), 3000);
      }
